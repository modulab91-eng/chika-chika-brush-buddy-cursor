...
1. **Google 계정 로그인**
   ...
   - 사용자 정보 자동 추출 (이름, 이메일, 프로필 사진)

2. **게스트 로그인**
   - 인증 없이 빠르게 앱 테스트 가능

3. **세션 관리**
   - localStorage를 통한 지속적 인증 상태 유지
   - 새로고침 후에도 로그인 상태 유지
   - 로그아웃 시 세션 자동 삭제

### Google OAuth 설정
자세한 설정 방법은 [GOOGLE_OAUTH_SETUP.md](./GOOGLE_OAUTH_SETUP.md)를 참고하세요.
...
- **React Router** - 라우팅
- **date-fns** - 날짜 처리
- **@react-oauth/google** - Google OAuth 통합
...
# .env.local 파일 생성 후 Google OAuth 클라이언트 ID 입력
# 상세 가이드: GOOGLE_OAUTH_SETUP.md 참고
...
│   ├── ProtectedRoute.tsx        # 보호된 라우트 (인증 확인)
│   └── ui/                       # shadcn/ui 컴포넌트
├── context/
